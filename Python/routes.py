...
def dfs(place, dist_so_far, roads, distances):
    ...
    elif distances[place] <= dist_so_far:
        return None
    else:
        distances[place] = dist_so_far
    for i in roads[place]:
        to_city, dist = i                                     #Separate tuple within roads[place]
        dfs(to_city, dist + dist_so_far, roads, distances)    #Call dfs with neighbors of place and add current dist to dist of neighbor
